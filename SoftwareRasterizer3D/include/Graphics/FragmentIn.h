...
public: // defaults but for clarity.
	...
	FragmentIn(int x, int y, float z, const Vec3<float>& vertNormal, const Vec3<float>& faceNormal, const Vec4<float>& colour) : x(x), y(y), z(z), pointNormal(vertNormal.GetNormalized()), colour(colour) {}
	FragmentIn(int x, int y, float z, const Vec3<float>& vertNormal, const Vec3<float>& faceNormal, const Vec4<float>& colour, const Vec2<float>& uv, const Vec3<float>& tangent, const Vec3<float>& bitangent)
		: x(x), y(y), z(z), pointNormal(vertNormal.GetNormalized()), faceNormal(faceNormal), colour(colour), uv(uv), tangent(tangent), bitangent(bitangent) {}
