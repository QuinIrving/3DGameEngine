...
public:
	...
	VertexPostClip(Vec3<float> position, float invW, Vec4<float> colour, Vec3<float> normal, Vec2<float> UV) 
		: position(position), invW(invW), colour(colour), normal(normal), UV(UV) {}
